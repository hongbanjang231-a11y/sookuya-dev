...
## 9. PM 결정사항 (v0.2 + v0.3 반영)

**v0.3 추가 (2026-05-25):**

| # | 항목 | 결정 | DB·정책 영향 |
|---|------|------|---------------|
| D10 | 사이클 단계 단순화 | 의견·투표·댓글 모두 사이클 전체 동안 자유 (요일별 윈도우 폐지) | `topics` 테이블의 `opinion_window_*`, `vote_window_*`, `comment_window_ends_at` 컬럼은 유지하되 항상 `cycle_starts_at`/`cycle_ends_at`과 동일. strict CHECK(`opinion_ends < vote_starts` 등) 제거하고 "윈도우는 cycle 안에만" 느슨한 CHECK로 교체. 마이그레이션 011에서 적용. 향후 컬럼 삭제 검토 (v1) |
